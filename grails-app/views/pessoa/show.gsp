	...
	<body>
		...
		<div id="show-pessoa" class="content scaffold-show" role="main">
			...
			<ol class="property-list pessoa">
				...
				<g:if test="${pessoaInstance?.books}">
				<li class="fieldcontain">
					<span id="books-label" class="property-label"><g:message code="pessoa.books.label" default="Books" /></span>
					
						<g:each in="${pessoaInstance.books}" var="b">
						<span class="property-value" aria-labelledby="books-label">
							<g:link controller="pessoaBook" action="show" id="${b?.id}">${b?.book.title}</g:link></span>
						</g:each>
